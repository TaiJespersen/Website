...
window.AstroShip = {
  ...
  warpTo(coords, onArrival) {
    ...
    this.targetPos = new THREE.Vector3(coords.x, coords.y, coords.z + 32);
  ...
  update(delta) {
    ...
    if (this.isAutopilot && this.targetPos) {
      // Smooth hyperbolic warp deceleration toward target
      ...
      if (dist < 4.0) {
        ...
        const warpSpeed = Math.min(180, Math.max(25, dist * 2.8));
        this.velocity.copy(toTarget.normalize()).multiplyScalar(warpSpeed);
      ...
      const targetRoll = -this.angularVelocity.y * 3.5;
